...
.DESCRIPTION
    This script performs the complete setup for the VCamSample virtual camera:
...
$SolutionFile = Join-Path $RepoRoot "VCamSample.sln"
$OutputDir = Join-Path $RepoRoot "$Platform\$Configuration"
$MSBuildPath = "C:\Program Files\Microsoft Visual Studio\2022\Community\MSBuild\Current\Bin\MSBuild.exe"
$NuGetExe = Join-Path $RepoRoot "nuget.exe"

Write-Host "VCamSample Build and Registration Script" -ForegroundColor Green
...
try {
    ...
    $dllPath = Join-Path $OutputDir "VCamSampleSource.dll"
    $exePath = Join-Path $OutputDir "VCamSample.exe"
    Test-FileExists $dllPath "VCamSampleSource.dll"
    Test-FileExists $exePath "VCamSample.exe"
    ...
    $targetExePath = Join-Path $TargetPath "VCamSample.exe"
    Test-FileExists $targetDllPath "Target VCamSampleSource.dll"
    Test-FileExists $targetExePath "Target VCamSample.exe"
    ...
    Write-Host ""
    Write-Host "SUCCESS: VCamSample setup completed!" -ForegroundColor Green
    Write-Host "========================================" -ForegroundColor Green
    ...
    Write-Host "3. Select 'VCam Sample Source' as camera input"
    Write-Host ""
